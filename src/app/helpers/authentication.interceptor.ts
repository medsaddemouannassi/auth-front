...
export class AuthenticationInterceptor implements HttpInterceptor {
  ...
  intercept(httpRequest: HttpRequest<any>, httpHandler: HttpHandler): Observable<HttpEvent<any>> {
    if (httpRequest.url.includes(`${this.authenticationService.host}/api/users/login`)) {
      return httpHandler.handle(httpRequest);
    }
    if (httpRequest.url.includes(`${this.authenticationService.host}/api/users/signup`)) {
      return httpHandler.handle(httpRequest);
    }
    ...
    if (httpRequest.url.includes(`${this.authenticationService.host}/api/users/refreshToken`)) {
    ...
    return httpHandler.handle(request).pipe(catchError(error => {
      if (error instanceof HttpErrorResponse && !httpRequest.url.includes('/api/users/login') && error.status === 401) {
